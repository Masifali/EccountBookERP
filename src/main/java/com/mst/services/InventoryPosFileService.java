...
import com.mst.repositories.InventoryPosItemRepository;
import java.io.*;
...
@Service
public class InventoryPosFileService {
    private final DesktopAttachmentStore store;private final JdbcTemplate jdbc;private final InventoryPosItemRepository repo;
    public InventoryPosFileService(DesktopAttachmentStore store,JdbcTemplate jdbc,InventoryPosItemRepository repo){this.store=store;this.jdbc=jdbc;this.repo=repo;}
    public String prepareProduct(UserAccount u,InventoryPosItemRequest r){if(r.productImage==null)return null;byte[] bytes=decode(r.productImage);imageType(bytes);return store.store(u,r.productImage.name,bytes);}
    public String prepareBarcode(UserAccount u,InventoryPosItemRequest r){if(r.barcodeImage==null)return null;byte[] bytes=decode(r.barcodeImage);if(!"image/png".equals(imageType(bytes)))throw new IllegalArgumentException("Barcode image must be PNG");return store.store(u,r.barcodeImage.name,bytes);}
    public void persist(UserAccount u,int item,int type,InventoryPosItemRequest r){
        ...
        for(var file:r.files){byte[] bytes=decode(file);String stored=store.store(u,file.name,bytes);Timestamp now=new Timestamp(System.currentTimeMillis());
            Object[] values={0,type,0,0,item,file.name,now,u.getId(),now,u.getId(),u.getOrganizationId(),u.getCompanyId(),0,"InvAddItemsPOS",false,stored,bytes.length/1048576d,0};
    ...
    public Download product(UserAccount u,int item){var row=repo.record(u,item);String name=Objects.toString(row.get("Pic1"),"");if(name.isBlank())throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.NOT_FOUND,"No product image");byte[] bytes=store.read(u,basename(name));return new Download(basename(name),bytes,imageType(bytes));}
