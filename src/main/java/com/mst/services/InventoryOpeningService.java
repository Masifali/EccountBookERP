...
public class InventoryOpeningService {
    ...
    private Map<String,Object> year(UserAccount u){List<Map<String,Object>> years=repo.years(u);if(years.isEmpty())throw new IllegalStateException("No active financial year is allocated to this company");int chosen=context.currentFinancialYearId();/* clsGlobalVariables.ActiveYr: the year chosen at login */return years.stream().filter(y->y.get("Id") instanceof Number n&&n.intValue()==chosen).findFirst().orElseThrow(()->new IllegalStateException("The financial year selected at login is not active for this company"));}
