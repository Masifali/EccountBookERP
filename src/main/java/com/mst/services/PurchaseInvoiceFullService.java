...
public class PurchaseInvoiceFullService {
    ...
    public int generateNextDocNo(int orgId, int compId, int branchId, int yearId, int docTypeId) {
        try {
            int dt = docTypeId > 0 ? docTypeId : 56;
            String sql = "SELECT ISNULL(MAX(DocNo), 0) + 1 FROM InvPurchaseInvoice WHERE DocumentTypeId = ? AND (OrganizationId = ? OR OrganizationId IS NULL) AND (CompanyId = ? OR CompanyId IS NULL)";
            Integer nextNo = jdbcTemplate.queryForObject(sql, Integer.class, dt, orgId, compId);
    ...
    public List<Map<String, Object>> getHistory(int orgId, int compId, int branchId, int yearId, int docTypeId,
        ...
        try {
            ...
            sb.append("SELECT i.Id as id, i.DocNo as docNo, CONVERT(VARCHAR(10), i.DocDate, 120) as docDate, ")
              ...
              .append("WHERE i.DocumentTypeId = ").append(dt).append(" ")
    ...
    public Map<String, Object> savePurchaseInvoice(Map<String, Object> payload) {
        ...
        try {
            ...
            Integer docTypeId = payload.get("documentTypeId") != null ? ((Number) payload.get("documentTypeId")).intValue() : 56;

            Integer docNo = payload.get("docNo") != null ? ((Number) payload.get("docNo")).intValue() : generateNextDocNo(orgId, compId, branchId, yearId, docTypeId);
            ...
            if (id > 0) {
                ...
                jdbcTemplate.update(insertHead, docTypeId, docNo, docDate, supplierId, billAmount, remarks, manualBillNo, orgId, compId, branchId, yearId);
            ...
            response.put("message", "Purchase Invoice saved successfully [" + docNo + "]");
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", "Error saving Purchase Invoice: " + e.getMessage());
        }
        return response;
    }
    ...
    public boolean deletePurchaseInvoice(int id) {
        try {
            jdbcTemplate.update("DELETE FROM InvPurchaseInvoiceDetail WHERE InvPurchaseInvoiceId=?", id);
            jdbcTemplate.update("DELETE FROM InvPurchaseInvoice WHERE Id=?", id);
            return true;
        } catch (Exception e) {
            return false;
        }
