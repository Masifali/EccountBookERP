...
import com.mst.services.ReportConfigService;
...
/** Admin Panel -> "Define Reports" (frmReportConfig). */
@Controller
public class ReportConfigController {

    private static final String TOKEN = "reportConfigCsrf";

    private final ReportConfigService service;
    public ReportConfigController(ReportConfigService service) { this.service = service; }

    @GetMapping("/configurations/define-reports")
    public String page(Model model) {
        model.addAttribute("canOpen", service.canOpen());
        return "configurations/report_config";
    }

    /** GridFill() - and the page's CSRF token, since Spring's own filter is disabled app-wide. */
    @GetMapping("/configurations/define-reports/api/history")
    @ResponseBody
    ...
    /** RetrivedData(Id). */
    @GetMapping("/configurations/define-reports/api/{id}")
    @ResponseBody
    ...
    /** Insert(). */
    @PostMapping("/configurations/define-reports/api/save")
    @ResponseBody
