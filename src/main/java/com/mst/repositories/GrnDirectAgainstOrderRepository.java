...
public class GrnDirectAgainstOrderRepository {
    ...
    public List<Map<String, Object>> getTransporters(int orgId, int compId) {
        try {
            String sql = "SELECT Id as id, CompanyName as name FROM SupplierCustomer WHERE CustomerGroupId = 7 AND (OrganizationId = ? OR OrganizationId IS NULL) AND (CompanyId = ? OR CompanyId IS NULL) ORDER BY CompanyName";
