...
public class DesktopInventoryCategoryRepository {
 ...
 private boolean config(UserAccount u,String name){var rows=jdbc.queryForList("EXEC dbo.Sp_ConfigrationsAllocation_GetAllMethod @OrganizationId=?, @CompanyId=?, @ConfigDescription=?, @Activity=?",u.getOrganizationId(),u.getCompanyId(),name,"GetConfigurationByOrgCompandConfigDescription");return !rows.isEmpty()&&toBool(rows.get(0).get("ConfigKey"));}
 /* Conversion.ToBool (architecture.common Conversion.cs:12-30): "True"/"False", else Convert.ToInt32 != 0, else false. ConfigKey can hold "1". */
 static boolean toBool(Object value){String s=Objects.toString(value,"").trim();if(s.equalsIgnoreCase("true"))return true;if(s.equalsIgnoreCase("false")||s.isEmpty())return false;try{return Integer.parseInt(s)!=0;}catch(NumberFormatException e){try{return new java.math.BigDecimal(s).signum()!=0;}catch(NumberFormatException e2){return false;}}}
