...
//		$("#gl_report_datatable span#cTotalQTY").text(cTotalQTY);
    ...
    // SUBMIT FORM
    $("#submitGrossWeight").on("click", function (event) {
        // Prevent the form from submitting via the browser.
        event.preventDefault();
        generateGLReport("/reports/gross_weight_stock_report_quantity");
    });
    $("#glReportForm").submit(function (event) {
        // Prevent the form from submitting via the browser.
        event.preventDefault();
        generateGLReport("/reports/stock_report_quantity");
    });

    function generateGLReport(url) {
        ...
        var table = $('#gl_report_datatable').DataTable({
            ...
            "footerCallback": function (row, data, start, end, display) {
                ...
                var colNumber = [4, 5, 7, 8, 9, 10, 11, 12, 13, 14];
            ...
            success: function (data) {
                ...
                $.each(data, function (i, sLEntry) {
                    ...
                    table.row.add([
                        ...
                        sLEntry.inQty,
                        sLEntry.avgPurchase,
                        sLEntry.purchasePrice,

                        sLEntry.outQty,
                        formatter.format(sLEntry.avgSale),
                        formatter.format(sLEntry.salePrice),
//                        sLEntry.prQty,
//                        sLEntry.srQty,
                        sLEntry.cQty,
                        formatter.format(sLEntry.stockAmount),
                        //formatter.format(sLEntry.salePrice - ((sLEntry.outQty / 40) * sLEntry.avgPurchase)),
                        formatter.format((sLEntry.purchasePrice + sLEntry.oAmount) - sLEntry.salePrice - sLEntry.stockAmount),
                    ]);
                });
