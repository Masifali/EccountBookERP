...
$(document).ready(function() {
	...
	$("#generateVoucher").on("click", function () {
		...
		window.open("/costing/closingJV?companyId=" + $("#companyId option:selected").val() + "&branchId=" + $("#branchId option:selected").val() + "&closingType=" + $("#closingType option:selected").val() + "&financialYearId=" + $("#financialYearId option:selected").val() + "&financialMonthId=" + $("#financialMonthId option:selected").val() + "", '_blank');
	});
...
//		    	totalFeet = parseFloat((totalFeet).toFixed(2));
	...
	// SUBMIT FORM
    $("#inventoryClosingForm").submit(function(event) {
		// Prevent the form from submitting via the browser.
		event.preventDefault();
		getSalesBreakdown();
