...
$(document).ready(function () {
    ...
    $("#printVoucherListPt").on("click", function (event) {
        window.open("/reports/printVoucherWithCommission/?voucherList=" + vouchersId + "&pageType=" + "a4", '_blank');
    });
    $("#printVoucherList").on("click", function (event) {
        window.open("/reports/printVoucherWithCommission/?voucherList=" + vouchersId, '_blank');
    });
    $("#printVoucherList").on("click", function (event) {
        window.open("/reports/printVoucherWithCommission/?voucherList=" + vouchersId, '_blank');
    });
    $("#twoVoucherONSinglePage").on("click", function (event) {
        window.open("/reports/print-voucher-list/?voucherList=" + vouchersId + "&bp=" + "3", '_blank');
    });
    ...
    var brokery = parseFloat(0);
    var safw8 = parseFloat(0);
    var totalAmount = parseFloat(0);
    var labour = parseFloat(0);
    ...
    function generateGLReport(whichButtonIsPressed) {
        ...
        var table = $('#gl_report_datatable').DataTable({
            dom: 'Bfrtip',
            deferRender: true,
            /* scrollY: 'auto',  // This makes the table's height automatic based on content
             scroller: false,  // Disable scroller (if it's not needed anymore)
             destroy: true,*/
            bPaginate: false,
            fixedHeader: true,
            ...
            success: function (data) {
                ...
                $.each(data, function (i, sLEntry) {
                    ...
                    if (whichButtonIsPressed != 2) {
                        ...
                        brokery = brokery + sLEntry.brokeryAmount;
                        labour = labour + sLEntry.labour;
                    ...
                    table.row.add([
                        (i + 1),
                        getFormattedDate(new Date(sLEntry.voucherDate)) + '<br/>' + getFormattedDate(new Date(sLEntry.modifyDate)),
                        sLEntry.millKhata,
                        sLEntry.id ? voucherViewLink : "",
                        sLEntry.bookNumber,
                        sLEntry.partyName,
                        sLEntry.truck,
                        formatVoucherNarration(description),

                        formatter.format(sLEntry.amount),
                        formatter.format(sLEntry.grossAmount),
                        sLEntry.remarks,
                    ]);
                });
                ...
                $("#submitGLForm").attr("disabled", false);
                $("#dublicate").attr("disabled", false);// enable submit button
    ...
    function setFootterValue() {
        ...
        $("span#des").text("Bag:" + bag + " W:" + formatter.format(safw8) + " M:" + formatter.format(mill) + " B:" + formatter.format(bank) + " F:" + formatter.format(fr) + " S:" + formatter.format(slai) + " BR:" + formatter.format(bar) + " Oth:" + formatter.format(other) + " Brokery:" + formatter.format(brokery) + " Labour" + formatter.format(labour));
