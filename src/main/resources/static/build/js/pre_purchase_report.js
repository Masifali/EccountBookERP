...
$(document).ready(function () {
    ...
    $("#editPrePurchaseEntryModel #prePurchaseReportForm").submit(function (event) {
        ...
        var formData = {
            ...
            paymentType: $("#prePurchaseReportForm #paymentType").val(),
            status: $("#prePurchaseReportForm #status").val(),
        }
        // DO POST
        $.ajax({
            type: "POST",
            ...
            success: function (data) {
                if (data === "saved") {
                    ...
                    $.confirm({
                        ...
                        //typeAnimated: true,
                        content: data,//.addClass('display').css('font-size', '12px'),
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i, pPEntry) {
                    var partyName = "";
                    var combineOrSingleVoucher = 0;
                    var voucherType = "PJV";
                    combineOrSingleVoucher = "/payable/new_pjv_qty_form/?prePuchaseEntryId=" + pPEntry.id;
                    if (pPEntry.weightEntryId == 0) {
                        combineOrSingleVoucher = "/SaleQty/new_pjv_sjv_qty_form/?prePuchaseEntryId=" + pPEntry.id;
                        voucherType = "PSJV";
                    } else if (pPEntry.ppvn.indexOf("KWEO") >= 0) {
                        voucherType = "SJV";
                        combineOrSingleVoucher = "/receivables/new_sjv_qty_form/?prePuchaseEntryId=" + pPEntry.id;
                    }
                    if ($("#hidePurchaser").val() === 'true') {
                        partyName = pPEntry.account;
                    }
                    table.row.add([
                        '<td>' + (i + 1) + '</td>',
                        '<td><a class="nn">' + pPEntry.ppvn + '</a></td>',
                        '<td>' + partyName + '<br />' + pPEntry.mill + '</td>',
                        '<td>' + pPEntry.item + '<br/>' + pPEntry.paymentType + '</td>',
                        '<td><a class="date">' + getFormattedDate(new Date(pPEntry.date)) + '</a> <input type="hidden" class = "entryDate" value ="' + getMyDate(new Date(pPEntry.date)) + '"></td>',
                        '<td><a class="vehicalNo">' + pPEntry.vehicalNo + '</a></td>',
                        '<td><a class="bag">' + pPEntry.bag + '</a> <input type= "hidden" class ="company" value=' + pPEntry.company + '></td>',
                        '<td><a class="kg">' + pPEntry.kg + '</a> <input type= "hidden" class ="branch" value=' + pPEntry.branch + '></td>',
                        '<td><a class="rate">' + pPEntry.rate + '</a> <input type= "hidden" class ="ppvn" value=' + pPEntry.ppvn + '></td>',
                        '<td><a class="freight">' + pPEntry.freight + '</a> <input type="hidden" class = "entryDate" value ="' + getMyDate(new Date(pPEntry.date)) + '"></td>',
                        '<td><a class="remarks">' + pPEntry.remarks + '</a> <input type= "hidden" class ="paymentType" value=' + pPEntry.paymentType + '></td>',
                        '<td><a class="status">' + pPEntry.secondWeight + '<br />' + pPEntry.firstWeight + '</a> </td>',
                        '<td><a class="status">' + pPEntry.status + '</a> </td>',
                        '<td><input type= "hidden" class ="id" value=' + pPEntry.id + '> <input type= "hidden" class ="itemDefId" value=' + pPEntry.itemId + '> <input type= "hidden" class ="millId" value=' + pPEntry.millId + '> <input type= "hidden" class ="code" value=' + pPEntry.code + '><a  class="editPrePurchaseEntry" style="text-decoration: underline;color:red" href="javascript:void(0)">  EDIT</a>/<a class="entry_delete" style="text-decoration: underline;color:green"  target=_blank href=' + location.protocol + "//" + location.host + combineOrSingleVoucher + ' >' + voucherType + '</a></td>',
    ...
    $('body').on('click', '.editPrePurchaseEntry', function () {
        ...
        $("#prePurchaseReportForm #paymentType").val($(this).closest("tr").find(".paymentType").val().trim());
        $("#prePurchaseReportForm #status").val($(this).closest("tr").find(".status").text().trim());
