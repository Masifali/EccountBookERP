    ...
    function generateGLReport(){
    	...
    	var table = $('#gl_report_datatable').DataTable({
	        ...
	        buttons: [
	            ...
	            {
	                extend: 'excelHtml5',
	                title: 'CUSTOMER STATUS REPORT' + ' (' + (new Date().toLocaleString()) + ')',
	                messageTop: null,
	                messageBottom: null,
	                footer: true,
	            	exportOptions: {
	            ...
	            },
	            {
	                extend: 'pdfHtml5',
	                download: 'open',
	                title: 'CUSTOMER STATUS REPORT' + ' (' + getFormattedDate(new Date()) + ", " + new Date().toLocaleString().split(",")[1].trim() + ')',
	                messageTop: null,
	                messageBottom: null,
	                footer: true,
	                exportOptions: {
    	...
    	$.ajax({
			...
			dataType : "json",
			success:function(data){
				//http://localhost:8033/reports/general_ledger?code=33121021012&mobileNumbers=+923004252766
				// FILL TABLE ROWS				
		        $.each(data, function (i, gLEntry) {
		        	
		    		table.row.add([
                        (i+1),
                        "<a style='text-decoration: underline;' target=_blank href="+location.protocol + "//" + location.host+"/reports/general_ledger/?code="+gLEntry.accountCode+"&mobileNumbers="+gLEntry.accountContactMobile.replace(", ", ",").replace("+92", "0").trim()+">"+gLEntry.accountFormattedCode+"</a>",
                        "<a style='text-decoration: underline;' target=_blank href="+"http://192.168.1.50:8080/BusinessPromotor/SearchContactByNumber.jsp?contactNumbers="+gLEntry.accountContactMobile.replace(", ", ",").replace("+92", "0").trim()+"&userid="+encodeURIComponent($("#userFullName").val())+"&name="+encodeURIComponent(gLEntry.accountName)+">"+gLEntry.accountName+"</a>",
