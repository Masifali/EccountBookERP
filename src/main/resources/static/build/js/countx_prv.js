...
Number.prototype.toFixed = function(decimalPlaces) {
	return oldToFixed.call((Math.round(this * 100) / 100), decimalPlaces);
}
...
$( document ).ready(function() {
	...
	$(".purchase_order_entry_add").on("click", function () {
	...
	$(".purchase_order_entry_delete").on("click", function () {
	...
	function loadItemDefDetail(thisControl) {
		...
		if (!$("#company").val() || !$("#itemStock\\.branch").val() || !$("#voucherStatus").val()){
			return;
		}
	...
	function calculateTotal(){
		...
		$("#purchase_order_entry_table tfoot tr:eq(0) th:eq(1)").text(totalWeight);
		$("#purchase_order_entry_table tfoot tr:eq(0) th:eq(3)").text(totalFeet);
		$("#purchase_order_entry_table tfoot tr:eq(0) th:eq(4)").text(formatter.format(amountTotal));
		
		$("#tax_table tbody tr:eq(0) th:eq(2)").text(formatter.format(amountTotal));
		var salesTaxAmount = parseFloat(calcPercentage(amountTotal, $("#tax_table tbody tr:eq(1) th:eq(1) input:first").val()).toFixed(2));
		salesTaxAmount = parseFloat((salesTaxAmount).toFixed(2));
		$("#tax_table tbody tr:eq(1) th:eq(2)").text(formatter.format(salesTaxAmount));
		
		var furtherSalesTaxAmount = parseFloat(calcPercentage(amountTotal, $("#tax_table tbody tr:eq(2) th:eq(1) input:first").val()).toFixed(2));
		furtherSalesTaxAmount = parseFloat((furtherSalesTaxAmount).toFixed(2));
		$("#tax_table tbody tr:eq(2) th:eq(2)").text(formatter.format(furtherSalesTaxAmount));
		
		var amountWithSalesTax = amountTotal + salesTaxAmount + furtherSalesTaxAmount;
		amountWithSalesTax = parseFloat((amountWithSalesTax).toFixed(2));
		$("#tax_table tbody tr:eq(3) th:eq(2)").text(formatter.format(amountWithSalesTax));
		
		var advanceIncomeTaxAmount = parseFloat(calcPercentage(amountWithSalesTax, $("#tax_table tbody tr:eq(4) th:eq(1) input:first").val()).toFixed(2));
		advanceIncomeTaxAmount = parseFloat((advanceIncomeTaxAmount).toFixed(2));
		$("#tax_table tbody tr:eq(4) th:eq(2)").text(formatter.format(advanceIncomeTaxAmount));
		$("#tax_table tbody tr:eq(5) th:eq(2)").text(formatter.format(amountWithSalesTax + advanceIncomeTaxAmount));
		
		$(".loadingCharges").val(($(".loadingRate").val() * totalWeight).toFixed(3));
		
		$("#voucher_entry_table tbody tr:eq(0) td:eq(4) input").val(parseFloat((amountWithSalesTax + advanceIncomeTaxAmount + parseFloat($(".loadingCharges").val())).toFixed(2)));
		$("#voucher_entry_table tbody tr:eq(1) td:eq(5) input").val(amountTotal);
		$("#voucher_entry_table tbody tr:eq(2) td:eq(5) input").val(salesTaxAmount);
		$("#voucher_entry_table tbody tr:eq(3) td:eq(5) input").val(furtherSalesTaxAmount);
		$("#voucher_entry_table tbody tr:eq(4) td:eq(5) input").val(advanceIncomeTaxAmount);
		$("#voucher_entry_table tbody tr:eq(5) td:eq(5) input").val($(".loadingCharges").val());
	...
	$(".unloadingRate").on("input", function () {
		$(".unloadingCharges").val(($(".unloadingRate").val() * $("#purchase_order_entry_table tfoot tr:eq(0) th:eq(1)").text()).toFixed(2));
    });
    ...
    /*$("#purchaseOrderForm").submit(function(event) {
		...
		addOrUpdatePurchaseOrderData();
	});*/
    
    function addOrUpdatePurchaseOrderData(){

    	/*if ($("#purchase_order_entry_table span#debitTotal").text() !== $("#voucher_entry_table span#creditTotal").text()){
    		$("#modal_notify h4.modal-title:first").text("DEBIT AMOUNT DOES NOT MATCH CREDIT AMOUNT");
        ...
        $("#purchase_order_entry_table tbody tr").each(function() {
        	if ($(this).find("td").length) {
            	...
            	var entry = {
                	id : $(this).find("#voucherEntry_id").val(),
                	branch : {id: $(this).find("#voucherEntry_branch option:selected").val()},
                	account : {code: $(this).find("#voucherEntry_account option:selected").val()},
                	voucherDate : $("#voucherForm #voucherDate").val(),
                	narration : $(this).find("#voucherEntry_narration").val(),
                	chequeNumber : $(this).find("#voucherEntry_chequeNumber").val(),
                	recBookNumber : $(this).find("#voucherEntry_recBookNo").val(),
                	recBookPageNumber : $(this).find("#voucherEntry_recBookPageNo").val(),
                	debit : $(this).find("#voucherEntry_debit").val(),
                	credit : $(this).find("#voucherEntry_credit").val(),
                	voucher: {}
                }
    	...
    	// PREPARE FORM DATA    
    	var formData = {
    		id : $("#purchaseOrderForm #voucher_id").val(),
    	...
    	$.ajax({
			...
			url : $('#purchaseOrderForm').attr("action"),
			...
			success:function(data){
				...
				if (data.errorMessage.length == 0){
					...
					// ERROR MESSAGE
					$("#modal_notify h4.modal-title:first").text(data.errorMessage);
		    		$("#modal_notify").modal("show");
				}
