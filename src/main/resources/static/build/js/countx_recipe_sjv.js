...
//
    ...
    $(".pjv_entry_add").on("click", function () {
        ...
        // again instrument select2 dropdowns
        $("#purchase_entry_table .select2_single").select2({dropdownAutoWidth: true, width: '100%'});

        // update serial numbers
        $("#purchase_entry_table span#serial_no").each(function (index, element) {
            $(element).text(index + 1);
...
//	    	$(thisControl).closest("tr").find(".pricingRule").val(data.pricingRule);
    ...
    $("#purchase_entry_table").on("input", "input", function () {
        calculateTotalSjv();
        calculateTotalPjv();
    });
    ...
    function calculateTotalPjv() {
        ...
        $("#purchase_entry_table tbody tr").each(function () {
            ...
            $(this).find(".percentage").val(0);
            $(this).find(".percentage40").val(0);
        ...
        $("#purchase_entry_table tfoot tr:eq(0) th:eq(3)").text(totalSafiKg);
        $("#purchase_entry_table tfoot tr:eq(0) th:eq(5)").text(amountTotal);
        $("#purchase_entry_table tfoot tr:eq(0) th:eq(7)").text(totalPercentage);
        $("#purchase_entry_table tfoot tr:eq(0) th:eq(6)").text(totalPercentage40);
        calculateAmountEntriesTotal();
    }
    ...
    function generateTabIndexing() {
        var tabIndex = 0;
        $("#purchase_entry_table tbody td").each(function (i) {
