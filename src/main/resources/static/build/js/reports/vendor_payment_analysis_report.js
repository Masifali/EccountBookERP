...
$(document).ready(function () {
    ...
    // SUBMIT FORM
    $("#vendorPaymentAnalysisReport").submit(function (event) {
        ...
        table = $('#sM_report_datatable').DataTable({
            ...
            buttons: [
                ...
                }),
                {
                    ...
                    pageSize: 'A4',
                    title: 'VENDOR PAYMENT ANALYSIS REPORT' + ' (' + getFormattedDate(new Date()) + ", " + new Date().toLocaleString().split(",")[1].trim() + ')',
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(dummyData, function (i, account) {
                    let totalSales = account.saleSum;
                    let salesCustomer = account.sales;
                    let salesPercentage = 0;
                    if (parseFloat(totalSales) !== 0 && parseFloat(salesCustomer) !== 0) {
                        salesPercentage = (parseFloat(salesCustomer) / parseFloat(totalSales)) * 100;
                    }
                    let receiptRate = 0;
                    let collectionRate = account.collection;
                    let openBalanceRate = account.opening;
                    let totalsRate = salesCustomer + openBalanceRate;
                    if (parseFloat(collectionRate) !== 0 && parseFloat(totalsRate) !== 0) {
                        receiptRate = (parseFloat(collectionRate) / parseFloat(totalsRate)) * 100;
                    }
                    //formatNum
                    table.row.add([
                        account.partyCode,
    ...
    function getExcelBuilder() {
        let xlsBuilder = {
            filename: 'VENDOR PAYMENT ANALYSIS REPORT ' + new Date().toLocaleString(),
            sheetName: 'sheet1',
            customize: function (xlsx) {
                ...
                $('row c ', sheet).each(
                    function () {
                        ...
                        let ind = parseInt(attr.substring(
                            1, attr.length));
                        ind = ind + downrows;
                        $(this).attr("r", pre + ind);
                    });
