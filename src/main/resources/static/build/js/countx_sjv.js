...
$(document).ready(function () {
    ...
    $(".entry_add").on("click", function () {
        ...
        $("#sale_order_entry_table span#serial_no").each(function (index, element) {
            ...
            $(element).closest("tr").find("input[name*='.unitValue']").attr("id", "itemStock.itemStockEntries" + index + ".unitValue");
            $(element).closest("tr").find("input[name*='.unitValue']").attr("name", "itemStock.itemStockEntries[" + index + "].unitValue");

            $(element).closest("tr").find("select[name*='.unitName']").attr("id", "itemStock.itemStockEntries" + index + ".unitName");
            $(element).closest("tr").find("select[name*='.unitName']").attr("name", "itemStock.itemStockEntries[" + index + "].unitName");
...
//			$(element).closest("tr").find( "input[name*='.discountAmount']" ).attr("name", "itemStock.itemStockEntries[" + index + "].discountAmount");
    ...
    $(".unitValue").on("change", function () {
        var selectedUnit = $(this).closest("tr").find(".unitValue option:selected").val();
        var itemQuantity = parseFloat($(this).closest("tr").find(".itemQuantity").val());
        var price = parseFloat($(this).closest("tr").find(".price").val());
        var conversionFactor = parseFloat($(this).closest("tr").find(".unitValue").data("conversion-factor")); // Get conversion factor dynamically

        // Make sure selectedUnit and itemQuantity are valid
        if (!isNaN(price) && !isNaN(conversionFactor) && selectedUnit) {
            if (selectedUnit == 1) {
                // Unit is "Bags" (value = 1), multiply by conversion factor to get equivalent KG
                $(this).closest("tr").find(".price").val((price * conversionFactor).toFixed(4)); // Convert Bags to KG
            } else {
                // Unit is "KG" (value = 40), divide by conversion factor to get equivalent Bags
                $(this).closest("tr").find(".price").val((price / conversionFactor).toFixed(4)); // Convert KG to Bags
                /* if (autioCall) {
                     $(this).closest("tr").find(".itemQuantity").val((itemQuantity * selectedUnit).toFixed(0));
                 }*/
            }
        }
        ...
        $("#sale_order_entry_table tbody tr").each(function () {
            ...
            if (!isNaN(price) && !isNaN(conversionFactor) && selectedUnit) {
                if (selectedUnit == 1) {
                    ...
                    $(this).closest("tr").find(".price").val((price / conversionFactor).toFixed(4)); // Convert KG to Bags
                    if (autioCall) {
                        $(this).closest("tr").find(".itemQuantity").val((conversionFactor * itemQuantity).toFixed(0));
                    }
                }
            }
            calculateTotal();
    ...
    function loadItemDefDetail(thisControl) {
        ...
        $.get("/receivables/item_def_detail?itemDefId=" + $(thisControl).val() + "&companyId=" + $("#company").val() + "&branchId=" + $("#branch").val() + "&voucherStatusId=" + '0', function (data) {
            ...
            $(thisControl).closest("tr").find(".price").val(data.saleRate);
            $(thisControl).closest("tr").find(".unitName").text(data.unitName);
            $(thisControl).closest("tr").find(".itemCategory").empty();
            $(thisControl).closest("tr").find(".unitValue").data("conversion-factor", data.conversionValue);
            $(thisControl).closest("tr").find(".unitValue").empty();
            var option = "<option value = " + data.unitValue + ">" + data.unitName + "</option>";
            $(thisControl).closest("tr").find(".unitValue").append(option);
            option = "<option value = " + data.conversionValue + ">" + data.conversionName + "</option>";
            $(thisControl).closest("tr").find(".unitValue").append(option);
            calculateTotal();
        });
    }
    ...
    $("#itemStock\\.account\\.code").on("change", function () {
        ...
        $.get("/receivables/account_limit_and_balance?accountCode=" + $("#itemStock\\.account\\.code").val() + "&voucherId=" + $("#voucherId").val(), function (data) {

            var balance = "0";
            balance = formatter.format(data.balanceBeforeVoucher);
            $("span#previousBalance").text(balance);
...
//	});
    ...
    function calculateTotal() {
        ...
        $("#sale_order_entry_table tfoot tr:eq(0) th:eq(8)").text(formatter.format(invoiceAmountTotal));
        ...
        var previousBalance = parseFloat($("span#previousBalance").text().replace(/[^0-9.-]/g, '') || 0).toFixed(2);
        $("span#balance").text(formatter.format(totalExpances + invoiceAmountTotal + previousBalance));
    ...
    $("#voucherForm").submit(function () {
        ...
        $("#sale_order_entry_table tbody tr").each(function () {
            ...
            if ((parseFloat($(this).find(".availableQuantity").text()) - parseFloat($(this).find(".itemQuantity").val())) < 0) {
                itemQtyZero = true;
            }
...
//    	}
    ...
    function generateTabIndexing() {
        ...
        $("#sale_order_entry_table tbody td").each(function (i) {
            ...
            } else if ($(this).find("select.unitValue").length) {
                tabIndex = tabIndex + 1;
                $(this).find("select.unitValue:eq(0)").attr('tabindex', tabIndex);
