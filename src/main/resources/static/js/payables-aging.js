...
(function () {
    ...
    async function api(path, options) {
        const r = await fetch('/api/accounts/payables-aging' + path, options);
    ...
    function filterAccounts() {
        ...
        const filtered = rawAccounts.filter(r => {
            const isPayable = Number(r.AccountClass || 1) === 1 || [2, 3, 14, 15].includes(Number(r.AccountTypeId));
            ...
            return isPayable && matchesParent;
        });
    ...
    async function refreshLookups() {
        ...
        try {
            ...
            const controls = (lookup.controls || []).filter(r => Number(r.TypeNo || 1) === 1 || Number(r.TypeNo || 1) === 2);
    ...
    function openVoucherModal(row, isOverdue) {
        ...
        el('voucherModalTitle').textContent = `Payable Aging Details (${isOverdue ? 'Over Due' : 'Not Yet Due'}) - ${row.AccountTitle || ''}`;
    ...
    function renderTable() {
        ...
        filteredRows.forEach(row => {
            const grp = row.ParentAccount || 'TRADE SUPPLIERS';
            if (currentGroup !== grp) {
        ...
        status(`${filteredRows.length} supplier account records displayed.`);
    }
    ...
    async function loadReport() {
        ...
        setLoading(true);
        status('Fetching Payables Aging Report...');
